...
3. Use its Decimal-derived `market`, `features`, `news_scoring`, `direction_counts`, `strategies`, `spy_regime`, `effective_strategy_weights`, `sizing`, `blocked_by`, and `research_candidates` directly for any proposal. `sizing.qty` already incorporates ATR risk, mandate headroom, signal strength, risk-off scale, and same-side correlation-cluster scaling. Sandbox code may be used autonomously for independent validation, data inspection, parser fixtures, and bounded research experiments, but must not replace these canonical proposal fields or create execution authority.
